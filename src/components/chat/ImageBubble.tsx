...
export const ImageBubble: React.FC<ImageBubbleProps> = ({ attachment }) => {
  ...
  return (
    <div className="flex flex-col gap-2 my-2.5 max-w-sm select-none">
      <div className="relative rounded-2xl overflow-hidden border border-zinc-800 bg-zinc-950/65 shadow-md group">
        <img
        ...
        <div className="absolute bottom-2 left-2 bg-black/70 px-2 py-0.5 rounded-md text-[9px] font-semibold text-zinc-400">
      ...
      {hasDetections && (
        <div className="rounded-xl border border-[rgba(255,255,255,0.06)] bg-[#111113] p-3 text-[11px] font-mono shadow-sm">
          <div className="flex items-center gap-1.5 border-b border-[rgba(255,255,255,0.04)] pb-1.5 mb-2 text-zinc-300">
          ...
          <div className="space-y-1 text-zinc-400">
            ...
            {attachment.suggestedActions && attachment.suggestedActions.length > 0 && (
              <div className="mt-2 pt-2 border-t border-[rgba(255,255,255,0.04)]">
                <span className="text-zinc-500 font-bold block mb-1">Suggested Next Actions:</span>
                <div className="flex flex-wrap gap-1">
                  {attachment.suggestedActions.slice(0, 3).map((act, i) => (
                    <span key={i} className="rounded bg-[#ff6b35]/10 text-[#ff6b35] border border-[#ff6b35]/25 px-1.5 py-0.5 text-[9px] font-semibold">
                      {act}
                    </span>
                  ))}
                </div>
